...
bootstrap_cdk() {
  ...
  # Update existing roles with CDK bootstrap permissions if not already done
  if aws iam get-role --role-name GitHubActionsPreviewStaging &> /dev/null; then
    if ! aws iam get-role-policy --role-name GitHubActionsPreviewStaging --policy-name CDKBootstrapAccess &> /dev/null; then
      echo -e "${BLUE}Adding CDK bootstrap permissions to GitHubActionsPreviewStaging role...${NC}"
      aws iam put-role-policy \
        --role-name GitHubActionsPreviewStaging \
        --policy-name CDKBootstrapAccess \
        --policy-document file://.aws-github-oidc/cdk-bootstrap-policy.json
  ...
  if aws iam get-role --role-name GitHubActionsProduction &> /dev/null; then
    if ! aws iam get-role-policy --role-name GitHubActionsProduction --policy-name CDKBootstrapAccess &> /dev/null; then
      echo -e "${BLUE}Adding CDK bootstrap permissions to GitHubActionsProduction role...${NC}"
      aws iam put-role-policy \
        --role-name GitHubActionsProduction \
        --policy-name CDKBootstrapAccess \
        --policy-document file://.aws-github-oidc/cdk-bootstrap-policy.json
